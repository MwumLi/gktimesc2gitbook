      ...
      res.on('end', () => {
        try {
          ...
          if (config.debug) {
            ...
            console.debug(
              `当前http状态码: ${res.statusMessage}, 当前响应 headers: \n`,
              res.headers
            )
          }
          throw err
...
function generateSummary(menus) {
  let menuStr = menus
    .map((menu) => `* [${menu.title}](./docs/${menu.id}.md)`)
    .join('\n')
...
async function selectCourse() {
  ...
  for (let i = 0; i < courses.length; i++) {
    const { article, title, author } = courses[i]
...
async function main(_config) {
  ...
  //
  config.gitbook = path.join(config.gitbook, intro.column_title)
